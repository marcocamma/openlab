...
class Reader(object):
    ...
    def set_new_filename(self):
        _ = now()
        self.last_logfname = self.fname + "_%04d%02d%02d_%02d%02d%02d.h5" % (
            _.year,
            _.month,
            _.day,
            _.hour,
...
def main():
    ...
    parser.add_argument("--ip", default="127.0.0.1", help="hostname to read from")
    parser.add_argument("--fname", default="auto", help="filename to use")
    parser.add_argument("--log_every", default="1m", help="time between file savings")
    parser.add_argument(
        "--log_size_MB",
        default=10,
        type=int,
        help="max size of logfile before starting a new one",
    )
